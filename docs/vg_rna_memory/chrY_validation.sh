...

{
  ...
  echo "# step 3: max RSS (KiB); Stage A profile of the same chromosome (old -v recipe): 5367492"
  for l in control_t1_a patched_t1 control_t32 patched_t32; do printf '%-13s %s KiB  wall %s\n' "$l" "$(rss_kib "$l")" "$(wall "$l")"; done
